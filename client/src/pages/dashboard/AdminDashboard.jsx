...
import { deleteCourse, fetchCourses } from "../../redux/slices/CourseSlice";
...
function AdminDashboard() {
  ...
  const { allUsersCount: userCount, subscribedUsersCount: subscribedCount } =
    useSelector((state) => state?.stat);
  const { allPayments, monthlySalesRecord } = useSelector(
    (state) => state?.razorpay
  );
  const { courseData: allCourses } = useSelector((state) => state?.courses);
